...
export function DashboardStats() {
  ...
  return <div className="space-y-6 bg-gray-950">
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start">
        <div className="flex flex-wrap gap-4 items-center">
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="space-y-2">
              ...
              <div className="flex">
                <Input id="start-date" value={startDateInput} onChange={e => setStartDateInput(e.target.value)} placeholder="DD/MM/AAAA" className="w-[150px]" />
                <Popover>
                  ...
                  <PopoverContent className="w-auto p-0 pointer-events-auto" align="start">
                    <CalendarComponent mode="single" selected={startDate} onSelect={setStartDate} initialFocus locale={ptBR} className="p-3 pointer-events-auto" />
                  </PopoverContent>
                </Popover>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex">
                <Input id="end-date" value={endDateInput} onChange={e => setEndDateInput(e.target.value)} placeholder="DD/MM/AAAA" className="w-[150px]" />
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" size="icon" className="ml-2">
                      <LucideCalendarIcon className="h-4 w-4" />
                  ...
                  <PopoverContent className="w-auto p-0 pointer-events-auto" align="start">
                    <CalendarComponent mode="single" selected={endDate} onSelect={setEndDate} initialFocus locale={ptBR} className="p-3 pointer-events-auto" />
                  </PopoverContent>
                </Popover>
              </div>
            </div>
        ...
        {isAdmin && <Select value={nivelAcesso} onValueChange={setNivelAcesso}>
            <SelectTrigger className="w-[180px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Membros da Equipe" value={activeTeamMembers} description={`${teamMembers.length} membros no total`} icon={<Users className="h-4 w-4 text-white" />} iconBg="bg-blue-500" />
        <StatCard title="Projetos" value={totalProjects} description={`${inProgressProjects} em progresso`} icon={<Briefcase className="h-4 w-4 text-white" />} iconBg="bg-green-500" />
        <StatCard title="Tarefas Concluídas" value={filteredTaskStats.completed} description={`${filteredTaskStats.completed + filteredTaskStats.todo + filteredTaskStats.inProgress + filteredTaskStats.review} tarefas no total`} icon={<CheckCircle className="h-4 w-4 text-white" />} iconBg="bg-purple-500" />
        <StatCard title="Em Progresso" value={filteredTaskStats.inProgress + filteredTaskStats.review} description="Tarefas atualmente em andamento" icon={<ClockIcon className="h-4 w-4 text-white" />} iconBg="bg-orange-500" />
      </div>
      ...
      {isManager && <>
          <Card>
            ...
            <CardContent className="space-y-4">
              {filteredDepartmentStats.map(dept => <DepartmentProgress key={dept.department} department={dept.department} tasksCompleted={dept.tasksCompleted} tasksTotal={dept.tasksTotal} color={dept.color} />)}
              {filteredDepartmentStats.length === 0 && <div className="text-center py-4 text-muted-foreground">
          ...
          {isAdmin && <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              return <div key={member.id} className="space-y-2">
                          <div className="flex justify-between items-center">
